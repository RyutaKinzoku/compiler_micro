...
expr_rec gen_if(expr_rec condition, expr_rec then_case, expr_rec else_case){
    ...
    fprintf(x86_code, "\t%s ", strcat(then_if, ":"));
    fprintf(x86_code, "%s ", extract(then_case));
    fprintf(x86_code, "%s\n", e_rec.name);
    //generate(strcat(then_if, ":"), extract(then_case), e_rec.name, "");
    fprintf(x86_code, "%s", "\tJmp ");
    fprintf(x86_code, "%s\n", continue_if);
    //generate("Jmp", continue_if, "", "");
    fprintf(x86_code, "\t%s", strcat(else_if, ":"));
    fprintffprintf(x86_code, "%s ", extract(else_case));
    fprintf(x86_code, "%s\n", e_rec.name);
    //generate(strcat(else_if, ":"), extract(else_case), e_rec.name, "");
    fprintf(x86_code, "\t%s", strcat(continue_if, ":"));
